...
const Blogs = () => {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Subscribed!", description: data.message || "You're now subscribed to our newsletter." });
      setNewsletterEmail("");
    } catch (err: any) {
      toast({ title: "Error", description: err.message || "Failed to subscribe. Please try again.", variant: "destructive" });
    } finally {
  ...
  const uploadImage = async (): Promise<string | null> => {
    // Image upload via file storage is not yet configured.
    // Return null for now; images can be added via URL in the future.
    return null;
  };
  ...
  <div className="container mx-auto px-6 relative z-10">
    ...
    >
      ...
      <motion.div
        ...
        className="mt-10 text-gold/60 text-sm tracking-[0.4em] uppercase"
      >
        ✦ Royal Knowledge ✦
      </motion.div>
      <br/>
      ...
      >
        {"Saffron Insights".split("").map((char, index) => (
          <motion.span
            key={index}
            variants={{
